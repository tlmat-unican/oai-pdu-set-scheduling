...
void RCconfig_verify(configmodule_interface_t *cfg, ngran_node_t node_type)
{
  ...
  if (NODE_IS_CU(node_type)) {
    ...
    verify_gnb_param_notset(gnbp, GNB_DL_SCHEDULER_IDX, GNB_CONFIG_STRING_DL_SCHEDULER);
...
void RCconfig_nr_macrlc(configmodule_interface_t *cfg)
{
  ...
  const bool dl_scheduler_param_set = config_isparamset(GNBParamList.paramarray[0], GNB_DL_SCHEDULER_IDX);
  const char *dl_scheduler = *GNBParamList.paramarray[0][GNB_DL_SCHEDULER_IDX].strptr;
  LOG_I(GNB_APP,
        "dl_scheduler raw value='%s' (%s in config file)\n",
        dl_scheduler,
        dl_scheduler_param_set ? "explicitly set" : "default value used");
  if (strcmp(dl_scheduler, "pf") == 0) {
    config.dl_scheduler_type = NR_DL_SCHED_PF;
  } else if (strcmp(dl_scheduler, "custom") == 0 || strcmp(dl_scheduler, "scheduler_custom") == 0) {
    config.dl_scheduler_type = NR_DL_SCHED_CUSTOM;
  } else {
    AssertFatal(false,
                "Invalid value for %s: '%s'. Valid values are: pf, custom\n",
                GNB_CONFIG_STRING_DL_SCHEDULER,
                dl_scheduler);
  }
  LOG_E(GNB_APP,
        "dl_scheduler resolved enum=%d (%s)\n",
        (int)config.dl_scheduler_type,
        config.dl_scheduler_type == NR_DL_SCHED_PF ? "NR_DL_SCHED_PF" : "NR_DL_SCHED_CUSTOM");
  if (config.disable_harq)
    LOG_W(GNB_APP, "\"disable_harq\" is a REL17 feature and is incompatible with REL15 and REL16 UEs!\n");
  LOG_I(GNB_APP,
        "CSI-RS %d, SRS %d, SINR:%d, 256 QAM %s, delta_MCS %s, maxMIMO_Layers %d, HARQ feedback %s, num DLHARQ:%d, num ULHARQ:%d, DL scheduler:%s\n",
        config.do_CSIRS,
        config.do_SRS,
        config.do_SINR,
