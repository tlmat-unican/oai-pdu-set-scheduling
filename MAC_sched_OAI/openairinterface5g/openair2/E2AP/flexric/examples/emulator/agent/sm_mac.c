...
sm_ag_if_ans_t write_ctrl_mac_sm(void const* data)
{
  ...
  // Log for debugging
  // Imprimir cabecera para debug
  printf("++++++++++++++ Received MAC Control Request ++++++++++++++\n");
  printf("  HDR: dummy = %d\n", ctrl->hdr.dummy);
  printf("  MSG: n_users = %d\n", ctrl->msg.n_users);
  printf("  MSG: v = %d\n", ctrl->msg.v);
  
  // --- IMPRIMIR ARRAY WQ ---
  printf("  MSG: wq = [ ");
  if (ctrl->msg.wq != NULL) {
      for (int i = 0; i < ctrl->msg.n_users; i++) {
          // Imprimimos cada uint8_t como un entero sin signo (%u)
          printf("%u ", ctrl->msg.wq[i]);
      }
  } else {
      printf("NULL");
  }
