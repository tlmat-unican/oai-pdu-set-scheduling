...
// #include "../../sm/mac_sched_sm/mac_sched_sm_id.h"
...
void sm_cb_mac(sm_ag_if_rd_t const* rd)
{
  ...
  for(uint32_t i = 0; i < data->msg.len_ue_stats; ++i){
      // log for debugging
      // printf("[SM CB MAC DEBUG] Received UE RNTI: %u | q1: %u | q2: %u | q3: %u\n", 
      //        data->msg.ue_stats[i].rnti,
      //        data->msg.ue_stats[i].q1,
      //        data->msg.ue_stats[i].q2,
      //        data->msg.ue_stats[i].q3);
      mac_ue_stats_impl_t tmp = cp_mac_ue_stats_impl(&data->msg.ue_stats[i]) ;
      // log for debugging
      // printf("[SM CB MAC DEBUG] UE RNTI: %u | q1: %u | q2: %u | q3: %u\n", 
      //        tmp.rnti,
      //        tmp.q1,
      //        tmp.q2,
      //        tmp.q3);
...
// int report_mac_sched_sm(global_e2_node_id_t* id, Interval inter_arg, mac_cb* handler)
// {
//   assert(id != NULL);
//   assert(handler != NULL);
...
//   sm_ans_xapp_t ans = report_sm_xapp_api(id, SM_MAC_SCHED_ID, (void*)period, sm_cb_mac);
//   assert(ans.success == true); 
//   return ans.u.handle;
// }

int control_mac_sm(global_e2_node_id_t* id, mac_ctrl_msg_t* ctrl)
{
  assert(id != NULL);
  assert(ctrl != NULL);

  printf("[SWIG Wrapper] Sending MAC control: n_users=%u, v=%u\n", ctrl->n_users, ctrl->v);

  // Debug: imprimir wq y wg
  if (ctrl->wq != NULL) {
    printf("[SWIG Wrapper] wq: [");
    for (uint32_t i = 0; i < ctrl->n_users; i++) {
      printf("%u", ctrl->wq[i]);
      if (i < ctrl->n_users - 1) printf(", ");
    }
    printf("]\n");
  }

  if (ctrl->wg != NULL) {
    printf("[SWIG Wrapper] wg: [");
    for (uint32_t i = 0; i < ctrl->n_users; i++) {
      printf("%u", ctrl->wg[i]);
      if (i < ctrl->n_users - 1) printf(", ");
    }
  ...
  // Preparar el mensaje de control
  mac_ctrl_req_data_t ctrl_req = {0};
  ctrl_req.hdr.dummy = 0;
  ctrl_req.msg = cp_mac_ctrl_msg(ctrl);  // Copia profunda del mensaje

  // Enviar el control request a través de la API
  sm_ans_xapp_t ans = control_sm_xapp_api(id, SM_MAC_ID, &ctrl_req);

  // Liberar memoria temporal
  free_mac_ctrl_msg(&ctrl_req.msg);

  // Verificar resultado
  if (ans.success) {
    printf("[SWIG Wrapper] MAC control sent successfully\n");
    return ans.u.handle;
  } else {
    printf("[SWIG Wrapper] Failed to send MAC control\n");
    return -1;
  }
}
// int control_mac_sched_sm(global_e2_node_id_t* id, mac_ctrl_msg_t* ctrl)
// {
//   assert(id != NULL);
//   assert(ctrl != NULL);
...
//   // Enviar el control request a través de la API
//   sm_ans_xapp_t ans = control_sm_xapp_api(id, SM_MAC_SCHED_ID, &ctrl_req);
...
// void rm_report_mac_sched_sm(int handle)
// {
