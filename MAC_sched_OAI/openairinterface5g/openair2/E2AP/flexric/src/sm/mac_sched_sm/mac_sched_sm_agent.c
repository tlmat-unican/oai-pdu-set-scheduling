...
#include "mac_sched_sm_agent.h"

#include "../../util/alg_ds/alg/defer.h"
#include "dec/mac_sched_dec_generic.h"
#include "mac_sched_sm_id.h"
#include "enc/mac_sched_enc_generic.h"
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
...
//  wr.ctrl.type = MAC_CTRL_REQ_V0; 
  ...
  mac_ctrl.msg.n_users = msg.n_users;
  mac_ctrl.msg.v = msg.v;
  mac_ctrl.msg.wq = msg.wq;
  mac_ctrl.msg.wg = msg.wg;
  
  printf("[MAC SM AGENT] Control received: n_users=%u v=%u\n", mac_ctrl.msg.n_users, msg.v);
  
  // Imprimir array wq
  printf("[MAC SM AGENT] wq: [");
  if (mac_ctrl.msg.wq != NULL && mac_ctrl.msg.n_users > 0) {
    for (uint8_t i = 0; i < mac_ctrl.msg.n_users; i++) {
      printf("%u", mac_ctrl.msg.wq[i]);
      if (i < mac_ctrl.msg.n_users - 1) printf(", ");
    }
  } else {
    printf("NULL");
  }
  ...
  // Imprimir array wg
  printf("[MAC SM AGENT] wg: [");
  if (mac_ctrl.msg.wg != NULL && mac_ctrl.msg.n_users > 0) {
    for (uint8_t i = 0; i < mac_ctrl.msg.n_users; i++) {
      printf("%u", mac_ctrl.msg.wg[i]);
      if (i < mac_ctrl.msg.n_users - 1) printf(", ");
    }
  } else {
    printf("NULL");
...
//  printf("[E2SM MAC] on_e2_setup called \n");
  ...
  size_t const sz = strnlen(SM_MAC_SCHED_STR, 256);
  ...
  memcpy(setup.ran_fun_def, SM_MAC_SCHED_STR , sz);
  ...
  setup.rf.id = SM_MAC_SCHED_ID;
  setup.rf.rev = SM_MAC_SCHED_REV;

  setup.rf.oid = calloc(1, sizeof(byte_array_t) );
  assert(setup.rf.oid != NULL && "Memory exhausted");

  *setup.rf.oid = cp_str_to_ba(SM_MAC_SCHED_OID);
*/
...
sm_agent_t* make_mac_sm_agent(sm_io_ag_ran_t io)
{
  ...
  // Read
  sm->base.io.read_ind = io.read_ind_tbl[MAC_SCHED_V0];
  sm->base.io.read_setup = io.read_setup_tbl[MAC_SCHED_AGENT_IF_E2_SETUP_ANS_V0];
 
  //Write
  sm->base.io.write_ctrl = io.write_ctrl_tbl[MAC_SCHED_CTRL_REQ_V0];
  sm->base.io.write_subs = io.write_subs_tbl[MAC_SCHED_SUBS_V0];
