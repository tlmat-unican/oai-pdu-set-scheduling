...
typedef struct
{
  ...
  // New fields
  // uint8_t dl_mcs3;     // MCS 3 (0-28)
  // uint8_t q1;      // Buffer queue 1
  // uint8_t q2;      // Buffer queue 2
  // uint8_t q3;      // Buffer queue 3
  uint8_t n_users;   // Número de usuarios (tamaño de los arrays) TODO cambiar a numero de flujos, porque ue_stats ya es array!
  uint8_t* queues;
  uint8_t* virtual_queues;
  uint8_t* mcs;
  uint8_t* sensing;
} mac_ue_stats_impl_t;
...
typedef struct {
  // uint32_t action;
  uint8_t n_users;       // Número de usuarios (tamaño del array actions)
  uint8_t v;
  uint8_t* wq;      // Array dinámico de acciones: actions[0], actions[1], ..., actions[n_users-1]
  uint8_t* wg;
} mac_ctrl_msg_t;
