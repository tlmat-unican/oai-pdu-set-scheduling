...
{
  assert(src != NULL);
  // Nada que liberar (solo tiene uint32_t dummy)
}
...
void free_mac_ind_msg(mac_ind_msg_t* src)
{
  ...
  if(src->len_ue_stats > 0){
    ...
    // 1. PRIMERO: Liberar lo de dentro (el array de queues de cada UE)
    for (size_t i = 0; i < src->len_ue_stats; i++) {
      // Verificamos si la cola existe antes de liberarla
      // (Nota: asegúrate de que 'queues' es un puntero dinámico. Si es estático no hace falta free)
    ...
    // 2. SEGUNDO: Liberar el contenedor principal
    free(src->ue_stats);
    src->ue_stats = NULL; // Buena práctica para evitar doble free
  }
}
...
mac_ue_stats_impl_t cp_mac_ue_stats_impl(mac_ue_stats_impl_t const* src)
{
  ...
  // Copiar todos los campos escalares y arrays fijos
  ...
  // Copiar arrays HARQ fijos
  for(uint32_t i = 0; i < 5; ++i){
    dst.dl_harq[i] = src->dl_harq[i];
    dst.ul_harq[i] = src->ul_harq[i];
  }
  
  // Inicializar queues a NULL EXPLÍCITAMENTE
  dst.queues = NULL;
  dst.virtual_queues = NULL;
  dst.mcs = NULL;
  dst.sensing = NULL;
    
  // Copiar array dinámico de queues (deep copy)
  if (src->n_users > 0 && src->queues != NULL) {
    dst.queues = calloc(src->n_users, sizeof(uint8_t));
    assert(dst.queues != NULL && "Memory exhausted");
    
    // printf("[CP] BEFORE: src->queues @ %p: [", (void*)src->queues);
    // for (uint8_t j = 0; j < src->n_users; j++) {
    //   printf("%u", src->queues[j]);
    //   if (j < src->n_users - 1) printf(", ");
    // }
    // printf("]\n");
    
    memcpy(dst.queues, src->queues, src->n_users * sizeof(uint8_t));
    
    // printf("[CP] AFTER:  dst.queues @ %p: [", (void*)dst.queues);
    // for (uint8_t j = 0; j < dst.n_users; j++) {
    //   printf("%u", dst.queues[j]);
    //   if (j < dst.n_users - 1) printf(", ");
    // }
    // printf("]\n");
  }

  // Copiar array dinámico de virtual_queues (deep copy)
  if (src->n_users > 0 && src->virtual_queues != NULL) {
    dst.virtual_queues = calloc(src->n_users, sizeof(uint8_t));
    assert(dst.virtual_queues != NULL && "Memory exhausted");
    memcpy(dst.virtual_queues, src->virtual_queues, src->n_users * sizeof(uint8_t));
  }
  // Copiar array dinámico de mcs (deep copy)
  if (src->n_users > 0 && src->mcs != NULL) {
    dst.mcs = calloc(src->n_users, sizeof(uint8_t));
    assert(dst.mcs != NULL && "Memory exhausted");
    memcpy(dst.mcs, src->mcs, src->n_users * sizeof(uint8_t));
  }
  // Copiar array dinámico de sensing (deep copy)
  if (src->n_users > 0 && src->sensing != NULL) {
    dst.sensing = calloc(src->n_users, sizeof(uint8_t));
    assert(dst.sensing != NULL && "Memory exhausted");
    memcpy(dst.sensing, src->sensing, src->n_users * sizeof(uint8_t));
  }
...
bool eq_mac_ind_msg(mac_ind_msg_t* m0, mac_ind_msg_t* m1)
{
  ...
  for(uint32_t i = 0 ; i < m0->len_ue_stats; ++i){
    ...
    if(
        ...
        ue0->frame != ue1->frame ||
        ue0->slot != ue1->slot ||
        ue0->n_users != ue1->n_users || // Comparar número de usuarios
        eq_float(ue0->pusch_snr, ue1->pusch_snr, 0.0000001) == false ||
        eq_float(ue0->pucch_snr, ue0->pucch_snr, 0.0000001) == false 
      )
      return false;
      // Comparar arrays de queues
      if (ue0->n_users > 0) {
        if (memcmp(ue0->queues, ue1->queues, ue0->n_users * sizeof(uint8_t)) != 0)
          return false;
      }
      // Comparar arrays de virtual_queues
      if (ue0->n_users > 0) {
        if (memcmp(ue0->virtual_queues, ue1->virtual_queues, ue0->n_users * sizeof(uint8_t)) != 0)
          return false;
      }
      // Comparar arrays de mcs
      if (ue0->n_users > 0) {
        if (memcmp(ue0->mcs, ue1->mcs, ue0->n_users * sizeof(uint8_t)) != 0)
          return false;
      }
      // Comparar arrays de sensing
      if (ue0->n_users > 0) {
        if (memcmp(ue0->sensing, ue1->sensing, ue0->n_users * sizeof(uint8_t)) != 0)
          return false;
      }
  }
  return true;
...
{
  assert(src != NULL);
  // Nada que liberar (solo contiene dummy)
}
...
{
  assert(src != NULL);
  // Nada que liberar (es un puntero a enum)
  free(src);
}
