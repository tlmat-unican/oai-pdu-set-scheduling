  ...
  for(uint32_t i = 0; i < ind_msg.len_ue_stats; ++i){
    ...
    if (ue->n_flows > 0) {
      if (ue->n_flows > 4) {
        ...
        printf("------------------ Warning: n_flows (%u) exceeds maximum allowed (4). Limiting to 4.\n", ue->n_flows);
        ue->n_flows = 4;
      }
      ue->queues = calloc(ue->n_flows, sizeof(uint8_t));
      assert(ue->queues != NULL);
      ue->virtual_queues = calloc(ue->n_flows, sizeof(uint8_t));
      assert(ue->virtual_queues != NULL);
      ue->mcs = calloc(ue->n_flows, sizeof(uint8_t));
      assert(ue->mcs != NULL);
      ue->sensing = calloc(ue->n_flows, sizeof(uint8_t));
      assert(ue->sensing != NULL);
      ue->bytes = calloc(ue->n_flows, sizeof(uint64_t));
      assert(ue->bytes != NULL);
      ue->rbs = calloc(ue->n_flows, sizeof(uint32_t));
      assert(ue->rbs != NULL);

      size_t queues_size = ue->n_flows * sizeof(uint8_t); // Todos los arrays tienen el mismo tamaño
      ...
      ptr += queues_size;
      size_t bytes_size = ue->n_flows * sizeof(uint64_t);
      memcpy(ue->bytes, ptr, bytes_size);
      ptr += bytes_size;
      size_t rbs_size = ue->n_flows * sizeof(uint32_t);
      memcpy(ue->rbs, ptr, rbs_size);
      ptr += rbs_size;
    } else {
      ue->queues = NULL;
    ...
    printf("++++++++++ Decoded UE %u: n_flows=%u, queues=[", i, ue->n_flows);
    for (uint8_t j = 0; j < ue->n_flows; j++) {
    ...
    printf("]\n");
    printf("++++++++++ Decoded UE %u: n_flows=%u, virtual_queues=[", i, ue->n_flows);
    for (uint8_t j = 0; j < ue->n_flows; j++) {
    ...
    printf("]\n");
    printf("++++++++++ Decoded UE %u: n_flows=%u, mcs=[", i, ue->n_flows);
    for (uint8_t j = 0; j < ue->n_flows; j++) {
      printf("%u", ue->mcs[j]);
      if (j < ue->n_flows - 1) {
        printf(", ");
    ...
    printf("]\n");
    printf("++++++++++ Decoded UE %u: n_flows=%u, sensing=[", i, ue->n_flows);
    for (uint8_t j = 0; j < ue->n_flows; j++) {
    ...
    printf("]\n");
    printf("++++++++++ Decoded UE %u: n_flows=%u, bytes=[", i, ue->n_flows);
    for (uint8_t j = 0; j < ue->n_flows; j++) {
      printf("%lu", ue->bytes[j]);
      if (j < ue->n_flows - 1) {
        printf(", ");
      }
    }
    printf("]\n");
    printf("++++++++++ Decoded UE %u: n_flows=%u, rbs=[", i, ue->n_flows);
    for (uint8_t j = 0; j < ue->n_flows; j++) {
      printf("%u", ue->rbs[j]);
      if (j < ue->n_flows - 1) {
        printf(", ");
