...
byte_array_t mac_enc_ind_msg_plain(mac_ind_msg_t const* ind_msg)
{
  ...
  for (uint32_t i = 0; i < ind_msg->len_ue_stats; i++) {
    // Size of basic struct
    total_size += sizeof(mac_ue_stats_impl_t);
    
    // Size of queues array
    total_size += ind_msg->ue_stats[i].n_flows * sizeof(uint8_t);
    // Size of virtual_queues array
    total_size += ind_msg->ue_stats[i].n_flows * sizeof(uint8_t);
    // Size of mcs array
    total_size += ind_msg->ue_stats[i].n_flows * sizeof(uint8_t);
    // Size of sensing array
    total_size += ind_msg->ue_stats[i].n_flows * sizeof(uint8_t);
    // Size of bytes array
    total_size += ind_msg->ue_stats[i].n_flows * sizeof(uint64_t);
    // Size of rbs array
    total_size += ind_msg->ue_stats[i].n_flows * sizeof(uint32_t);
  }
  ...
  for(uint32_t i = 0; i < ind_msg->len_ue_stats; ++i){
    ...
    // Copy complete struct (includes n_users but not the queues pointer)
    memcpy(ptr, ue, sizeof(mac_ue_stats_impl_t));
    ptr += sizeof(mac_ue_stats_impl_t);
    
    // Copy queues array
    if (ue->n_flows > 0 && ue->queues != NULL) {
      size_t queues_size = ue->n_flows * sizeof(uint8_t);
      memcpy(ptr, ue->queues, queues_size);
      ptr += queues_size;
    }
    // Copy virtual_queues array
    if (ue->n_flows > 0 && ue->virtual_queues != NULL) {
      size_t vqueues_size = ue->n_flows * sizeof(uint8_t);
      memcpy(ptr, ue->virtual_queues, vqueues_size);
      ptr += vqueues_size;
    }
    // Copy mcs array
    if (ue->n_flows > 0 && ue->mcs != NULL) {
      size_t mcs_size = ue->n_flows * sizeof(uint8_t);
      memcpy(ptr, ue->mcs, mcs_size);
      ptr += mcs_size;
    }
    // Copy sensing array
    if (ue->n_flows > 0 && ue->sensing != NULL) {
      size_t sensing_size = ue->n_flows * sizeof(uint8_t);
      memcpy(ptr, ue->sensing, sensing_size);
      ptr += sensing_size;
    }
    // Copy bytes array
    if (ue->n_flows > 0 && ue->bytes != NULL) {
      size_t bytes_size = ue->n_flows * sizeof(uint64_t);
      memcpy(ptr, ue->bytes, bytes_size);
      ptr += bytes_size;
    }
    // Copy rbs array
    if (ue->n_flows > 0 && ue->rbs != NULL) {
      size_t rbs_size = ue->n_flows * sizeof(uint32_t);
      memcpy(ptr, ue->rbs, rbs_size);
      ptr += rbs_size;
    }
  }
