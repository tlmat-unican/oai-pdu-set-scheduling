...
static const nr_rlc_sdu_segment_t *get_hol_sdu_segment(const nr_rlc_entity_t *rb)
{
  if (rb == NULL)
    return NULL;

  if (rb->stats.mode == NR_RLC_AM) {
    const nr_rlc_entity_am_t *am_entity = (const nr_rlc_entity_am_t *)rb;
    if (am_entity->retransmit_list != NULL)
      return am_entity->retransmit_list;
    return am_entity->tx_list;
  }

  if (rb->stats.mode == NR_RLC_UM) {
    const nr_rlc_entity_um_t *um_entity = (const nr_rlc_entity_um_t *)rb;
    return um_entity->tx_list;
  }

  const nr_rlc_entity_tm_t *tm_entity = (const nr_rlc_entity_tm_t *)rb;
  return tm_entity->tx_list;
}
...
mac_rlc_status_resp_t nr_mac_rlc_status_ind(const uint16_t ue_id, const frame_t frame, const logical_chan_id_t channel_idP)
{
  ...
  if (rb != NULL) {
    ...
    const nr_rlc_sdu_segment_t *hol_sdu;
    rb->set_time(rb, get_nr_rlc_current_time());
    ...
    hol_sdu = get_hol_sdu_segment(rb);
    if (hol_sdu != NULL) {
      ret.head_sdu_creation_time = frame;
      ret.head_sdu_remaining_size_to_send = hol_sdu->size;
      ret.head_sdu_is_segmented = !(hol_sdu->is_first && hol_sdu->is_last);
    }
  } else {
...
uint32_t nr_mac_rlc_hol_wait_us_ind(const uint16_t ue_id, const logical_chan_id_t channel_idP)
{
  uint32_t hol_wait_us = 0;
  ...
  if (rb != NULL) {
    const uint64_t now_us = time_average_now();
    rb->set_time(rb, get_nr_rlc_current_time());
    const nr_rlc_sdu_segment_t *hol_sdu = get_hol_sdu_segment(rb);
    if (hol_sdu != NULL && hol_sdu->sdu != NULL && hol_sdu->sdu->time_of_arrival > 0) {
      const uint64_t arrival_us = hol_sdu->sdu->time_of_arrival;
      if (now_us > arrival_us) {
        const uint64_t delta_us = now_us - arrival_us;
        hol_wait_us = (delta_us > UINT32_MAX) ? UINT32_MAX : (uint32_t)delta_us;
      }
    }
  }
