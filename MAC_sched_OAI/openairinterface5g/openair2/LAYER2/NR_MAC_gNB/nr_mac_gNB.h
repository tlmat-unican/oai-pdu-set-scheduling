...
typedef enum {
  NR_DL_SCHED_PF = 0,
  NR_DL_SCHED_CUSTOM = 1,
} nr_dl_scheduler_type_t;
...
typedef struct nr_mac_config_t {
  ...
  bool do_SINR;
  nr_dl_scheduler_type_t dl_scheduler_type;
} nr_mac_config_t;
...
// New structure for E2 MAC control parameters
typedef struct nr_e2_mac_ctrl_params {
  uint32_t v;
  uint32_t wq;
  uint32_t wg;
} nr_e2_mac_ctrl_params_t;
...
typedef struct {
  ...
  /// Virtual thput queue (for DL MAC scheduling)
  uint64_t virtual_thput_queue[NR_MAX_NUM_LCID];
  nr_e2_mac_ctrl_params_t e2_mac_ctrl;
} NR_UE_sched_ctrl_t;
