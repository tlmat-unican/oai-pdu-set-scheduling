  ...
  if (bid < ESM_DATA_EPS_BEARER_MAX) {
    ...
    if (ebr != NULL) {
      ...
      if (qos != NULL) {
        ...
        ebr->qos = *qos;
        LOG_I(MAC, "UE %d: Set bearer %d QCI %d, GBR UL %d, GBR DL %d, MBR UL %d, MBR DL %d\n",
              ueid, ebr->ebi, ebr->qos.qci, ebr->qos.gbrUL, ebr->qos.gbrDL,
              ebr->qos.mbrUL, ebr->qos.mbrDL);
      } else {
        LOG_I(MAC, "UE %d: No GFBR was provided\n", ueid);
      }
