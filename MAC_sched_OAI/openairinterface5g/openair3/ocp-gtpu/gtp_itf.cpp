...
typedef struct gtpv1u_bearer_s {
  ...
  uint8_t         npduNum;
  int outgoing_qfi;
  uint8_t         last_rx_dscp;                 ///< DSCP from last received inner IP packet
} gtpv1u_bearer_t;
...
static  int udpServerSocket(openAddr_s addr) {
  ...
  if (setsockopt(sockfd, IPPROTO_IP, IP_RECVTOS, &on, sizeof(on)) != 0)
    LOG_W(GTPU, "[%d] Could not enable IP_RECVTOS (%s)\n", sockfd, strerror(errno));
...
/*! \brief Get the DSCP value from the last received inner IP packet for a given bearer
 *  \param[in] instance GTP-U instance ID
 *  \param[in] ue_id UE ID
 *  \param[in] rb_id DRB ID (radio bearer ID)
 *  \return DSCP value (0-63) or 0 if not available
 */
uint8_t gtpv1u_get_dscp(instance_t instance, ue_id_t ue_id, int rb_id) {
  uint8_t dscp = 0;
  pthread_mutex_lock(&globGtp.gtp_lock);

  auto get_dscp_from_ue = [&](const teidData_t &ue_data) {
    auto rb_it = ue_data.bearers.find(rb_id);
    if (rb_it != ue_data.bearers.end())
      return rb_it->second.last_rx_dscp;

    // Fallback: bearer key used by caller may not match tunnel map key
    // (for example DRB ID vs PDU session ID). Return any non-zero DSCP for UE.
    for (const auto &b : ue_data.bearers) {
      if (b.second.last_rx_dscp > 0)
        return b.second.last_rx_dscp;
    }
    return (uint8_t)0;
  };

  auto inst = globGtp.instances.find(compatInst(instance));
  if (inst != globGtp.instances.end()) {
    auto ue_it = inst->second.ue2te_mapping.find(ue_id);
    if (ue_it != inst->second.ue2te_mapping.end())
      dscp = get_dscp_from_ue(ue_it->second);
  } else {
    // Fallback for callers passing a non-GTP instance identifier (for example module_id).
    // We scan all GTP instances and return the first matching UE/bearer DSCP.
    for (const auto &inst_kv : globGtp.instances) {
...
static int Gtpv1uHandleGpdu(int h,
  ...
  // Extract and store DSCP from the inner (decapsulated) IP packet carried in GTP-U payload.
  uint8_t inner_dscp = 0;
  if (sdu_buffer_size >= 2) {
    uint8_t ip_version = sdu_buffer[0] >> 4;
    if (ip_version == 4 && sdu_buffer_size >= 20) {
      uint8_t tos = sdu_buffer[1];
      inner_dscp = tos >> 2;
      LOG_I(GTPU, "[%d] RX inner IPv4 DSCP=%u (TOS=0x%02x)\n", h, inner_dscp, tos);
    } else if (ip_version == 6 && sdu_buffer_size >= 40) {
      uint8_t traffic_class = ((sdu_buffer[0] & 0x0F) << 4) | ((sdu_buffer[1] & 0xF0) >> 4);
      inner_dscp = traffic_class >> 2;
      LOG_I(GTPU, "[%d] RX inner IPv6 DSCP=%u (TC=0x%02x)\n", h, inner_dscp, traffic_class);
    }
  }

  // Store DSCP in bearer while still holding the lock
  auto bearer_it = inst->ue2te_mapping.find(tunnel->second.ue_id);
  // LOG_I(GTPU, "[DEBUG!] Looking for UE %lu to store last RX DSCP=%u\n", tunnel->second.ue_id, inner_dscp); ///
  if (bearer_it != inst->ue2te_mapping.end()) {
    auto pdu_it = bearer_it->second.bearers.find(tunnel->second.pdusession_id);
    if (pdu_it != bearer_it->second.bearers.end()) {
      pdu_it->second.last_rx_dscp = inner_dscp;
    }

    // MAC scheduler queries DSCP by DRB ID, so mirror value on incoming_rb_id too.
    auto drb_it = bearer_it->second.bearers.find(tunnel->second.incoming_rb_id);
    if (drb_it != bearer_it->second.bearers.end()) {
      drb_it->second.last_rx_dscp = inner_dscp;
    }

    if (pdu_it != bearer_it->second.bearers.end() || drb_it != bearer_it->second.bearers.end()) {
      LOG_I(GTPU,
            "[DEBUG!] Stored last RX DSCP=%u for UE %lu (PDU session %d, DRB %d)\n",
            inner_dscp,
            tunnel->second.ue_id,
            tunnel->second.pdusession_id,
            tunnel->second.incoming_rb_id);
    }
  }
...
void gtpv1uReceiver(int h) {
  ...
  uint8_t cmsgbuf[CMSG_SPACE(sizeof(uint8_t))];
  struct iovec iov = {0};
  struct msghdr msg = {0};
  from_len = (socklen_t)sizeof(struct sockaddr_in);

  iov.iov_base = udpData;
  iov.iov_len = sizeof(udpData);
  ...
  if ((udpDataLen = recvmsg(h, &msg, 0)) < 0) {
    ...
    for (struct cmsghdr *cmsg = CMSG_FIRSTHDR(&msg); cmsg != NULL; cmsg = CMSG_NXTHDR(&msg, cmsg)) {
      if (cmsg->cmsg_level == IPPROTO_IP && cmsg->cmsg_type == IP_TOS) {
        ...
        uint8_t dscp = tos >> 2;
        LOG_I(GTPU, "[%d] RX outer IP DSCP=%u (TOS=0x%02x)\n", h, dscp, tos);
        break;
      }
    }
